...
順帶一個可重複使用的觀察：`'blockperm'` 在 BlockLen=504（n=2000，僅 4 個區塊）時虛無 Sharpe 的相異值恰為 **24 = 4!**，多樣性完全塌陷如預期，但校準並未因此變差（KS 0.095、虛無 sd 0.416 對上其他組態的 ~0.44）。設計此類掃描時應同時記錄虛無統計量的相異值個數，才能分辨「多樣性不足」與「構造有偏」兩種失效。
