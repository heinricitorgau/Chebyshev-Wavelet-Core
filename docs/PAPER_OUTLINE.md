...
| **POM projection** — `pom_projection` | **Proved** (matches `pomProjection` 7.6e-14) |
| **Integrability** — `integrable_wavelet_mul_wavelet` | **Proved** (the `InWeightedL2` side condition) |
  ...
  **The $M_{\text{blk}}$ block is now proved too, so the whole OMI is verified.** Its content is the partial integral $\int_{\text{cellLower}}^{t}\psi_{n,m}$ re-expanded in the cell basis, which needs $T_{m+1}$ written back in the $U$ basis. Mathlib's `two_mul_T_eq_U_sub_U` gives $2T_{m+1} = U_{m+1} - U_{m-1}$, and because `U_neg_one` is zero this holds **uniformly in $m$** — at $m=0$ it degenerates correctly to $2T_1 = U_1$, so the proof needs no case split. The resulting three terms are exactly the three nonzero entries of a row of $M_{\text{blk}}$:
  $$\frac{+1}{2^{J+2}(m+1)}\ \text{on}\ \psi_{m+1},\qquad \frac{-1}{2^{J+2}(m+1)}\ \text{on}\ \psi_{m-1},\qquad \frac{(-1)^m}{2^{J+1}(m+1)}\ \text{on}\ \psi_{0}.$$
  Cross-checked over $k=1..4$, $M=2..6$: 360 entries, **maximum deviation 0.000e+00**.
  ...
  **The projection characterisation and the integrability side condition are now proved as well, which closes the formalisation of the numerical kernel.** `pom_projection` states that against every retained basis function $\psi_c$, the POM's expansion $\sum_a \tilde F[a,b]\,\psi_a$ has the same weighted inner product as $f\cdot\psi_b$ — the orthogonal-projection statement, in the form that avoids subtracting a residual. It rests on exactly the two results that were open a step earlier: `withinCellOrthonormal` for $\langle\psi_a,\psi_c\rangle = \delta_{ac}$, and integrability to move the finite sum through the integral. Integrability itself needs no bound on $|U_m|$: the integrand is an indicator of a continuous function on a bounded cell.

  **The hypothesis $c \in \{0,\dots,M-1\}$ is the truncation, not a technicality.** The residual is orthogonal to *retained* degrees only. That is precisely why MATLAB's `Verify` checks `pomProjection` against the projection integrals rather than against $f\cdot\psi$ pointwise — the same point that made the naive pointwise check in §3.0 fail at 74% error. The formalisation and that discarded check are two views of one fact.

  **Verification status: 961 comparisons, all matching** (`verify/verify_lean_agreement.m`), with `Nblk`, `Mblk`, $\Lambda$ and POM symmetry exact to `0.000e+00`.
...
5. **Lean 4** (§3.0). Convention gap **closed**; cross-cell orthogonality, `chebyshevU_orthogonality` and `withinCellOrthonormal` all **proved**, so the MATLAB normalisation is machine-verified. **The entire OMI — both `Nblk` and `Mblk` — is proved and cross-checked against MATLAB to 0.000e+00** (`verify/verify_lean_agreement.m`). **The POM core is proved too** — symmetry, block-diagonality, and the linearisation $U_lU_j=\sum_r U_{l+j-2r}$, the last matching MATLAB’s $\Lambda$ tensor to 0.000e+00. **921 comparisons across the whole numerical kernel, all matching.** The **projection characterisation** and the **integrability** side condition are now proved too, closing the formalisation of the numerical kernel: **961 comparisons, all matching.**
